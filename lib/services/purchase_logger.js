...
// Logger för Purchase Logger
const logger = createLogger('PurchaseLogger');
...
function readPurchaseLog() {
    ...
    try {
        ...
        logger.error('Fel vid läsning av köplog', { error: e.message });
        return { purchases: [], dailyTotals: {} };
    }
}
...
function logPurchase(purchaseData) {
    ...
    logger.info('Köp loggat', { 
        id: purchase.id, 
        amountSEK: purchase.amountSEK, 
        orgnr: purchase.orgnr 
    });
    logger.info('Dagens total', { 
        spent: log.dailyTotals[today], 
        limit: CONFIG.DAILY_LIMIT_SEK 
    });
