...
// Proxy-stöd via https-proxy-agent
let HttpsProxyAgent;
try {
    HttpsProxyAgent = require('https-proxy-agent').HttpsProxyAgent;
} catch (e) {
    // Fallback om paketet inte finns
    HttpsProxyAgent = null;
}

/**
 * Hämta proxy-agent om proxy är konfigurerad
 */
function getProxyAgent() {
    ...
    try {
        ...
        console.error('[Proxy] Kunde inte skapa proxy-agent:', e.message);
        return null;
    }
}
...
class BolagsverketOpenAPI {
    ...
    /**
     * Gör HTTPS-request med proxy-stöd
     */
    _request(url, options = {}) {
        return new Promise((resolve, reject) => {
            ...
            // Lägg till proxy-agent om tillgänglig
            if (agent) {
...
// Ger tillgång till utökad företagsdata via autentiserat API
// ============================================================================
...
class BolagsverketOAuthAPI {
    ...
    static TOKEN_MARGIN_MS = 5 * 60 * 1000; // Förnya 5 min innan utgång
    ...
    async _getToken() {
        if (!this.isConfigured) {
            console.error('[OAuth] Credentials ej konfigurerade');
            return null;
        }

        // Returnera cachad token om giltig
        if (this._accessToken && this._tokenExpiresAt) {
            if (Date.now() < this._tokenExpiresAt - BolagsverketOAuthAPI.TOKEN_MARGIN_MS) {
        ...
        try {
            ...
            console.log('[OAuth] Hämtar ny token...');
            const response = await this._request(BolagsverketOAuthAPI.TOKEN_URL, {
                method: 'POST',
            ...
            if (response.status !== 200) {
                console.error(`[OAuth] Token-fel: HTTP ${response.status}`);
                return null;
            }
            ...
            console.log(`[OAuth] ✅ Token erhållen, giltig i ${expiresIn}s`);
            return this._accessToken;

        } catch (error) {
            console.error('[OAuth] Token-fel:', error.message);
            return null;
        }
    }
    ...
    async _apiRequest(endpoint, method = 'POST', body = null) {
        ...
        const url = `${BolagsverketOAuthAPI.API_BASE_URL}${endpoint}`;
        const options = {
            method,
            headers: {
    ...
    /**
     * Hämta företagsinfo via OAuth API
     */
    async getCompany(orgnr) {
        const formatted = this._formatOrgnr(orgnr);
        return this._apiRequest('/foretag', 'POST', {
            organisationsnummer: formatted
        });
    }

    /**
     * Hämta dokumentlista via OAuth API
     */
    async getDocumentList(orgnr) {
        const formatted = this._formatOrgnr(orgnr);
        return this._apiRequest('/dokument/lista', 'POST', {
            organisationsnummer: formatted
        });
    }

    /**
     * Ladda ner dokument via OAuth API
     */
    async downloadDocument(dokumentId) {
        const token = await this._getToken();
        if (!token) return null;

        const url = `${BolagsverketOAuthAPI.API_BASE_URL}/dokument/${dokumentId}`;
        try {
            const response = await this._request(url, {
                method: 'GET',
                headers: {
     ...
     */
    async ping() {
        return this._apiRequest('/ping', 'GET');
    }
}
...
class BolagsverketVDMClient {
    ...
    get hasOAuthCredentials() {
        return this.oauthApi.isConfigured;
    }

    // =========================================================================
    // OAUTH METODER (kräver credentials)
    // =========================================================================

    /**
     * Hämta företagsinfo via OAuth API (utökad data)
     */
    async getCompanyOAuth(orgnr) {
        return this.oauthApi.getCompany(orgnr);
    }

    /**
     * Hämta dokumentlista via OAuth API
     */
    async getDocumentListOAuth(orgnr) {
        return this.oauthApi.getDocumentList(orgnr);
    }

    /**
     * Ladda ner dokument via OAuth API
     */
    async downloadDocumentOAuth(dokumentId) {
        return this.oauthApi.downloadDocument(dokumentId);
    }
