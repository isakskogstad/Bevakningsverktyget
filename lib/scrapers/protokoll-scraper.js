...
async function fetchProtokoll(orgnr, email, options = {}) {
    ...
    try {
        ...
        if (is3DSecure) {
            ...
            // Validera att 3D Secure lösenord finns
            if (!SECURE_PASSWORD) {
                const errorMsg = '3D Secure lösenord saknas! Sätt SECURE_3D_PASSWORD i miljövariabler.';
                console.error(`[3DS] ❌ ${errorMsg}`);
                steps.push({ step: 11, status: 'failed', message: errorMsg });
                ...
                return {
                    ...
                    email: email,
                    steps: steps,
                    error: 'SECURE_3D_PASSWORD_MISSING',
                    message: errorMsg
                };
            }
