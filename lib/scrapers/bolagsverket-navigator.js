...
const { BolagsverketVDMClient } = require('./bolagsverket_vdm');
const fs = require('fs');
...
/**
 * Försöker hämta årsredovisning GRATIS via Bolagsverket VDM API
 * VDM API tillhandahåller digitalt inlämnade årsredovisningar utan kostnad
 *
 * @param {string} orgnr - Organisationsnummer
 * @param {object} options - { savePath, saveToSupabase }
 * @returns {object} { success, documentPath, source: 'vdm_api', ... }
 */
async function fetchAnnualReportViaVDM(orgnr, options = {}) {
    const { savePath, saveToSupabase = true } = options;

    if (!vdmClient.isConfigured) {
        log('VDM API ej konfigurerat (saknar credentials)', 'info');
        return { success: false, error: 'VDM API not configured' };
    }

    try {
        log(`Kontrollerar gratis årsredovisningar via VDM API för ${orgnr}...`);

        // Hämta dokumentlista
        const docs = await vdmClient.getDocumentList(orgnr);
        ...
        if (arsredovisningar.length === 0) {
            log('Ingen gratis årsredovisning tillgänglig via VDM API');
            return { success: false, error: 'No free annual report available' };
        }

        // Ta senaste årsredovisningen
        const latestDoc = arsredovisningar[0];
        log(`Hittade gratis årsredovisning: ${latestDoc.rakenskapsperiod || 'okänd period'}`);

        // Ladda ner PDF
        const pdfData = await vdmClient.downloadDocument(latestDoc.dokumentId);
        if (!pdfData) {
            return { success: false, error: 'Failed to download document' };
        }

        // Spara lokalt
        const filename = `${orgnr}_arsredovisning_${latestDoc.rakenskapsperiod || 'latest'}.pdf`;
        ...
        log(`✅ Årsredovisning sparad (GRATIS via VDM): ${filePath}`, 'success');
        ...
        const result = {
            ...
            dokumentId: latestDoc.dokumentId,
            rakenskapsperiod: latestDoc.rakenskapsperiod,
            message: 'Hämtad gratis via Bolagsverket VDM API'
        };
            ...
            if (db) {
                try {
                    await db.from('purchased_documents').insert({
                        orgnr,
                        product_code: 'ARS',
                        product_name: 'Årsredovisning',
                        document_path: filePath,
                        price_sek: 0,
                        source: 'vdm_api',
                        metadata: { vdm_dokument_id: latestDoc.dokumentId, period: latestDoc.rakenskapsperiod }
                    });
        ...
        log(`VDM API-fel: ${error.message}`, 'error');
