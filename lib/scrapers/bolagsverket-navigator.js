...
const captchaSolver = require('../captcha/auto_captcha_solver');
const purchaseLogger = require('../services/purchase_logger');
const { BolagsverketVDMClient, DOCUMENT_TYPES } = require('./bolagsverket_vdm');
const fs = require('fs');
const path = require('path');

// VDM API client för gratis årsredovisningar
const vdmClient = new BolagsverketVDMClient();
...
/**
 * Försöker hämta årsredovisning GRATIS via Bolagsverket Öppet API
 * Använder det helt öppna API:et - INGEN autentisering krävs!
 *
 * Tillgängliga format:
 * - PDF: Komplett årsredovisning
 * - XBRL: Maskinläsbar för finansiell analys
 * - iXBRL: HTML+XML hybrid
 *
 * @param {string} orgnr - Organisationsnummer
 * @param {object} options - { savePath, saveToSupabase, format, year }
 * @returns {object} { success, documentPath, source: 'open_api', cost: 0, ... }
 */
async function fetchAnnualReportViaVDM(orgnr, options = {}) {
    const {
        savePath,
        saveToSupabase = true,
        format = 'pdf',  // 'pdf', 'xbrl', eller 'ixbrl'
        year = null      // null = senaste tillgängliga
    } = options;

    try {
        log(`Hämtar GRATIS årsredovisning via Öppet API för ${orgnr}...`);

        // Hämta alla tillgängliga årsredovisningar
        const arsInfo = await vdmClient.getAllArsredovisningar(orgnr);

        if (!arsInfo.arsredovisningar || arsInfo.arsredovisningar.length === 0) {
            log('Ingen årsredovisning tillgänglig via Öppet API');
            return { success: false, error: 'No annual report available', cost: 0 };
        }

        // Välj år (senaste om inte specificerat)
        const targetYear = year || arsInfo.arsredovisningar[0].ar;
        log(`Laddar ner årsredovisning för år ${targetYear} (${format.toUpperCase()})...`);

        // Ladda ner och spara
        const downloadDir = savePath || CONFIG.DOWNLOAD_DIR;
        const downloadResult = await vdmClient.downloadAndSave(orgnr, targetYear, format, downloadDir);

        if (!downloadResult.success) {
            log(`Nedladdning misslyckades: ${downloadResult.error}`, 'error');
            return downloadResult;
        }

        log(`✅ Årsredovisning sparad (GRATIS): ${downloadResult.filePath}`, 'success');

        const result = {
            success: true,
            source: 'open_api',
            cost: 0,
            documentPath: downloadResult.filePath,
            format: downloadResult.format,
            year: targetYear,
            foretagsnamn: arsInfo.foretagsnamn,
            availableYears: arsInfo.arsredovisningar.map(a => a.ar),
            message: `Hämtad GRATIS via Bolagsverket Öppet API (${format.toUpperCase()})`
        };

        // Spara till Supabase om aktiverat
        if (saveToSupabase) {
            const db = getSupabase();
            if (db) {
                try {
                    await db.from('purchased_documents').insert({
                        orgnr,
                        product_code: 'ARS',
                        product_name: `Årsredovisning ${targetYear}`,
                        document_path: downloadResult.filePath,
                        price_sek: 0,
                        source: 'open_api',
                        metadata: {
                            year: targetYear,
                    ...
                    log('Sparad till Supabase');
                } catch (dbErr) {
                    log(`Kunde inte spara till Supabase: ${dbErr.message}`, 'error');
                }
            }
        }
        ...
        log(`Öppet API-fel: ${error.message}`, 'error');
        return { success: false, error: error.message, cost: 0 };
    }
}

/**
 * Hämta alla tillgängliga årsredovisningar för ett företag (GRATIS)
 * @param {string} orgnr - Organisationsnummer
 * @returns {object} { success, arsredovisningar, downloadLinks, foretagsnamn }
 */
async function listAvailableArsredovisningar(orgnr) {
    return vdmClient.getAllArsredovisningar(orgnr);
}
...
async function purchaseDocument(orgnr, productCode, options = {}) {
    ...
    // === GRATIS ALTERNATIV: Försök hämta årsredovisning via VDM API först ===
    if (productCode === 'ARS' && vdmClient.isConfigured) {
        log('Försöker hämta årsredovisning GRATIS via VDM API...');
        const vdmResult = await fetchAnnualReportViaVDM(orgnr, { saveToSupabase });
        if (vdmResult.success) {
            log('✅ Årsredovisning hämtad gratis via VDM API!', 'success');
            return {
                success: true,
                orgnr,
                productCode,
                ...
                source: 'vdm_api',
                cost: 0,
                documentPath: vdmResult.documentPath,
                rakenskapsperiod: vdmResult.rakenskapsperiod,
                message: 'Hämtad gratis via Bolagsverket VDM API (ingen kostnad)'
            };
        }
        log('VDM API hade ingen gratis version, fortsätter med köp...');
    }
...
module.exports = {
    ...
    // VDM API (gratis årsredovisningar)
    fetchAnnualReportViaVDM,
    listAvailableArsredovisningar,
    vdmClient,
    DOCUMENT_TYPES,  // Exportera dokumenttyper (gratis vs kostar)
...
MILJÖVARIABLER:
  ...
  HEADLESS               Kör i headless-läge (default: false för CLI, true för API)
  SKIP_ADBLOCKER         Hoppa över adblocker-plugin (default: false)
`);
    };

    // Läs headless från miljövariabel (för serverless/CI-miljöer)
    const cliHeadless = process.env.HEADLESS === 'true';
    ...
    (async () => {
        ...
        if (mode === 'discover') {
            const results = await discoverAllProducts(orgnr, { headless: cliHeadless });
            console.log('\n📊 SAMMANFATTNING:');
        ...
        // Default: info
        const result = await getCompanyDocuments(orgnr, { headless: cliHeadless });
        console.log('\n📄 RESULTAT:');
        console.log(JSON.stringify(result, null, 2));
