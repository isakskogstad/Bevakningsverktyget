 ...
 *
 * Användning:
 *   const { setupPopupBlocker, dismissAllPopups, injectCookieBlocker } = require('./popup-blocker');
 *
...
async function waitForCaptchaResolution(page, maxWaitMs = 10000) {
    ...
    while (Date.now() - startTime < maxWaitMs) {
        ...
        if (hasCaptcha && !captchaFound) {
            captchaFound = true;
            console.error('[PopupBlocker] CAPTCHA upptäckt - väntar på lösning...');
        }
    ...
    if (captchaFound) {
        console.error('[PopupBlocker] CAPTCHA-timeout - fortsätter ändå...');
    }
