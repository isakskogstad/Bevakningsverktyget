 ...
 *
 * Usage:
 *   const { createBrowser, createPage, configurePage } = require('./browser-factory');
 *
...
// Logger för BrowserFactory
const logger = createLogger('BrowserFactory');

// puppeteer-real-browser för Cloudflare Turnstile bypass
let puppeteerRealBrowser = null;
try {
    puppeteerRealBrowser = require('puppeteer-real-browser');
} catch (e) {
    logger.warn('puppeteer-real-browser ej tillgänglig');
}
...
function configurePuppeteer() {
    ...
    // Stealth plugin (alltid aktivt)
    puppeteer.use(StealthPlugin());

    // Adblocker plugin med cookie notice blocking
    // Kan misslyckas om nätverksåtkomst till GitHub saknas
    try {
        // Kontrollera om vi ska hoppa över adblocker (för snabbare uppstart eller vid nätverksproblem)
        const skipAdblocker = process.env.SKIP_ADBLOCKER === 'true';
        if (!skipAdblocker) {
        ...
        logger.error('Kunde inte ladda Adblocker plugin', { error: e.message });
    }

    isConfigured = true;
    logger.info('Puppeteer konfigurerad med Stealth' + (adblockerEnabled ? ' + Adblocker' : ''));
}

/**
 * Hämta proxy-konfiguration från miljövariabler
 * Returnerar { server, username, password } eller null
 */
function getProxyConfig() {
    ...
    try {
        ...
        logger.error('Kunde inte parsa proxy URL', { error: e.message });
        return null;
    }
}

/**
 * Hitta Chrome/Chromium executable
 * Prioriterar: CHROMIUM_PATH env > Playwright installation > System chrome
 */
function findChromePath() {
    ...
    // 2. Playwright-installation (vanligt i CI/CD)
    const playwrightPaths = [
        '/root/.cache/ms-playwright/chromium-1194/chrome-linux/chrome',
        '/root/.cache/ms-playwright/chromium-1200/chrome-linux/chrome',
        path.join(process.env.HOME || '', '.cache/ms-playwright/chromium-1194/chrome-linux/chrome')
    ];
    for (const p of playwrightPaths) {
        if (fs.existsSync(p)) return p;
    }

    // 3. System Chrome (låt Puppeteer hitta själv)
    return undefined;
}
...
async function createBrowser(options = {}) {
    ...
    if (hasExtension) {
        ...
        logger.info('NopeCHA CAPTCHA-solver laddad');
    }

    // Extensions kräver GUI-mode
    const useHeadless = headless && !hasExtension;

    // Hitta Chrome executable automatiskt
    const chromePath = findChromePath();
    if (chromePath) {
        logger.info('Använder Chrome', { path: chromePath });
    }

    // Hämta proxy-konfiguration
    const proxyConfig = getProxyConfig();
    if (proxyConfig) {
        args.push('--proxy-server=' + proxyConfig.server);
        logger.info('Proxy konfigurerad', { server: proxyConfig.server });
    }
    ...
    logger.info('Browser startad', { headless: useHeadless });

    return browser;
}

/**
 * Skapar en "real browser" med puppeteer-real-browser
 * SPECIFIKT för Cloudflare Turnstile bypass
 *
 * @param {Object} options - Browser-alternativ
 * @param {boolean} options.headless - Kör headless (default: false för Turnstile)
 * @param {boolean} options.turnstile - Auto-lösa Cloudflare Turnstile (default: true)
 * @returns {Promise<{browser: Browser, page: Page}>} Browser och page
 */
async function createRealBrowser(options = {}) {
    if (!puppeteerRealBrowser) {
        throw new Error('puppeteer-real-browser är inte installerad. Kör: npm i puppeteer-real-browser');
    }

    const {
        headless = false, // Turnstile fungerar bäst i GUI-mode
        turnstile = true, // Auto-lösa Cloudflare Turnstile
        fingerprint = true, // Unik fingerprint för varje session
        args = []
    } = options;

    logger.info('Startar Real Browser med Turnstile-stöd');
    ...
    try {
        const { browser, page } = await puppeteerRealBrowser.connect({
            ...
            disableXvfb: false // Använd Xvfb på Linux för headless
        });

        logger.info('Real Browser startad', { headless, turnstile });

        // Konfigurera page med extra headers
        ...
        logger.error('Real Browser fel', { error: e.message });
        throw e;
    }
}
...
async function createPage(browser, options = {}) {
    const {
        ...
        extraHeaders = {},
        useProxy = true  // Aktivera proxy-autentisering automatiskt
    } = options;
    ...
    // Hantera proxy-autentisering om konfigurerad
    if (useProxy) {
        const proxyConfig = getProxyConfig();
        if (proxyConfig && proxyConfig.username && proxyConfig.password) {
            try {
                ...
                logger.info('Proxy-autentisering konfigurerad');
            } catch (e) {
                logger.error('Proxy-autentisering misslyckades', { error: e.message });
            }
        }
    }
...
async function saveCookies(page, filePath, domain = null) {
    ...
    logger.info('Cookies sparade', { count: cookies.length, path: filePath });
}
...
async function loadCookies(page, filePath, maxAgeMs = 7 * 24 * 60 * 60 * 1000) {
    try {
        ...
        if (Date.now() - data.savedAt > maxAgeMs) {
            logger.warn('Cookies för gamla, ignorerar');
            return false;
        }

        await page.setCookie(...data.cookies);
        logger.info('Cookies laddade', { count: data.cookies.length });
        return true;
    } catch (e) {
        logger.error('Fel vid laddning av cookies', { error: e.message });
        return false;
    }
}
...
async function takeScreenshot(page, name, dir = '/tmp') {
    ...
    logger.info('Skärmdump', { path: filepath });
    return filepath;
}
...
/**
 * Enkel funktion för att hämta HTML från en URL
 * Hanterar automatiskt popups, cookies och CAPTCHA
 *
 * @param {string} url - URL att hämta
 * @param {Object} options - Alternativ
 * @returns {Promise<{html: string, cookies: string}>} HTML och cookies
 */
async function fetchPage(url, options = {}) {
    ...
    try {
        await navigateAndConfigure(page, url, options);
        await handleCaptcha(page);

        const html = await page.content();
        const cookies = await exportCookieString(page, new URL(url).hostname);
...
/**
 * Hämtar och extraherar data med en custom scraper-funktion
 *
 * @param {string} url - URL att hämta
 * @param {Function} scraper - Funktion som tar (page) och returnerar data
 * @param {Object} options - Alternativ
 * @returns {Promise<any>} Data från scraper
 */
async function scrape(url, scraper, options = {}) {
    const browser = await createBrowser(options);
    const page = await createPage(browser, options);
...
module.exports = {
    // Browser & Page
    createBrowser,
    createRealBrowser, // NY: puppeteer-real-browser med Turnstile-stöd
    createPage,
    configurePage,
